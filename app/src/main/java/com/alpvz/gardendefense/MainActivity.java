...
smallAt(c,"PF "+plantFood,getWidth()*.87f,getHeight()*.045f,Color.WHITE);
...
void drawPeas(Canvas c){for(Pea q:peas){if(bulletImg!=null)c.drawBitmap(bulletImg,null,new RectF(q.x-10,q.y-10,q.x+10,q.y+10),p);else{p.setColor(Color.GREEN);c.drawCircle(q.x,q.y,7,p);}}}
...
void updatePlants(long now){for(int r=0;r<ROWS;r++)for(int col=0;col<COLS;col++){Plant a=plants[r][col];if(a==null)continue;if(a.type==SUNFLOWER&&now-a.lastAction>=7000){sunDrops.add(new SunDrop(left+col*cw+cw/2,top+r*ch+ch*.25f));a.lastAction=now;}if(a.type==PEASHOOTER&&now-a.lastAction>=1500&&rowHasZombie(r)){fire(r,col,30,0);a.lastAction=now;}if(a.type==REPEATER&&now-a.lastAction>=1800&&rowHasZombie(r)){fire(r,col,28,0);a.secondShotAt=now+500;a.lastAction=now;}if(a.type==REPEATER&&a.secondShotAt>0&&now>=a.secondShotAt){fire(r,col,28,0);a.secondShotAt=0;}if(a.type==CHOMPER&&now-a.lastAction>=3500){Zombie z=near(r,col);if(z!=null){z.hp=0;a.lastAction=now;}}if(a.type==MINE&&now>=a.armedAt){Zombie z=cellZombie(r,col);if(z!=null){z.hp=0;a.hp=0;}}}}
void fire(int row,int col,int damage,int type){peas.add(new Pea(left+col*cw+cw*.55f,top+row*ch+ch*.50f,row,damage));}
void updatePeas(float dt){Iterator<Pea>it=peas.iterator();while(it.hasNext()){Pea q=it.next();q.x+=cw*9f*dt;Zombie hit=null;for(Zombie z:zombies)if(z.row==q.row&&Math.abs(z.x-q.x)<cw*.28f){hit=z;break;}if(hit!=null){hit.hp-=q.damage;it.remove();}else if(q.x>getWidth()+30)it.remove();}}
void updateZombies(long now,float dt){for(Zombie z:zombies){if(z.hp<=0)continue;if(z.boss){z.x+=z.dir*cw*1.4f*dt;float min=left+cw*2,max=getWidth()-cw;if(z.x<min){z.x=min;z.dir=1;}if(z.x>max){z.x=max;z.dir=-1;}if(now-z.bossShot>=2200){fireBoss(z);z.bossShot=now;}}else{Plant targetPlant=plantAt(z);if(targetPlant!=null){if(now-z.lastAttack>=800){targetPlant.hp-=z.damage;z.lastAttack=now;}}else z.x-=z.speed*dt;}if(!z.boss&&z.x<left-cw*.4f){Mower m=mowers[z.row];if(!m.used){m.used=true;m.active=true;m.x=left-cw*.5f;}else{screen=LOSE;return;}}}}
...
void winLevel(){screen=WIN;coins+=mowerReward();if(level<LEVELS)unlocked=Math.max(unlocked,level+1);}
...
// Nhặt sun.
Iterator<SunDrop> si=sunDrops.iterator();while(si.hasNext()){SunDrop sd=si.next();if(Math.hypot(x-sd.x,y-sd.y)<38){sun+=25;si.remove();invalidate();return true;}}
...
float fx=getWidth()*.54f;if(x>=fx&&x<=fx+getWidth()*.09f&&plantFood>0){tool=PF_TOOL;return true;}}
if(x>=left&&x<left+COLS*cw&&y>=top&&y<top+ROWS*ch){int col=(int)((x-left)/cw),row=(int)((y-top)/ch);if(tool==SHOVEL){plants[row][col]=null;tool=NONE;return true;}if(tool==PF_TOOL){if(plants[row][col]!=null&&plantFood>0){usePF(plants[row][col]);plantFood--;}tool=NONE;return true;}if(plants[row][col]==null&&plantUnlocked(selected)){int co=cost(selected);if(sun>=co){sun-=co;plants[row][col]=new Plant(selected,row,col);}}invalidate();return true;}return true;}
void usePF(Plant a){if(a.type==SUNFLOWER)sun+=125;else if(a.type==PEASHOOTER){fire(a.row,a.col,70,0);fire(a.row,a.col,70,0);fire(a.row,a.col,70,0);}else if(a.type==GIGANUT)a.hp=a.maxHp;else if(a.type==CHOMPER){Zombie z=near(a.row,a.col);if(z!=null)z.hp=0;}else if(a.type==REPEATER){for(int i=0;i<6;i++)fire(a.row,a.col,55,0);}else if(a.type==MINE){for(Zombie z:zombies)if(z.row==a.row)z.hp=0;}}
...
class Plant{final int type,row,col,maxHp;int hp;long lastAction,secondShotAt,armedAt;Plant(int type,int row,int col){this.type=type;this.row=row;this.col=col;maxHp=type==GIGANUT?1800:type==MINE?120:500;hp=maxHp;long n=System.currentTimeMillis();lastAction=n;armedAt=type==MINE?n+3000:Long.MAX_VALUE;}}
class Zombie{float x,y,speed,dir=-1;final int row,type;final boolean boss;float hp,maxHp;int damage;long lastAttack,slowUntil,bossShot;Zombie(int row,int type,boolean boss){this.row=row;this.type=type;this.boss=boss;x=left+COLS*cw+cw;y=top+row*ch+ch*.16f;if(boss){maxHp=5000;speed=cw*.008f;damage=45;}else if(type==1){maxHp=1000+level*100;speed=cw*.010f;damage=32;}else if(type==2){maxHp=450+level*60;speed=cw*.024f;damage=18;}else{maxHp=650+level*75;speed=cw*.014f;damage=24;}hp=maxHp;bossShot=System.currentTimeMillis();}}
class Pea{float x;final float y;final int row,damage;Pea(float x,float y,int row,int damage){this.x=x;this.y=y;this.row=row;this.damage=damage;}}
