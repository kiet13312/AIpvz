...
public class MainActivity extends Activity {
    ...
    class GameView extends View {
        ...
        @Override protected void onDraw(Canvas c){layout(); if(screen==0)drawWorld(c);else drawGame(c); if(screen==1&&!paused&&!win&&!lose){update();postInvalidateDelayed(30);}}
        void layout(){left=12;top=getHeight()*.23f;cw=(getWidth()-24)/9f;ch=(getHeight()-top-8)/5f;}
        ...
        void startLevel(int l){level=Math.max(1,Math.min(MAX,l));screen=1;paused=false;win=false;lose=false;houseZombie=null;selected=PEA;skill=0;sun=500;food=3;spawned=0;killed=0;zs.clear();shots.clear();mines.clear();coins.clear();resetMowers();last=spawnAt=levelStart=System.currentTimeMillis();invalidate();}
        ...
        void useFood(int r,int c){if(food<=0||plants[r][c]==null)return;Plant a=plants[r][c];food--;a.pf=8;if(a.type==GIGA){a.max=8000;a.hp=8000;}if(a.type==CHOMP){a.pfMode=true;a.pull=4;a.timer=0;}}
        ...
        @Override public boolean onTouchEvent(MotionEvent e){if(e.getAction()!=MotionEvent.ACTION_UP)return true;float x=e.getX(),y=e.getY();
            ...
            float w=getWidth()/8f;if(y>=30&&y<=108){int pick=(int)(x/w);if(pick<=5){if(pick==0&&unlocked>=1)selected=SUN;else if(pick==1)selected=PEA;else if(pick==2&&unlocked>=2)selected=GIGA;else if(pick==3&&unlocked>=4)selected=CHOMP;else if(pick==4&&unlocked>=5)selected=REP;else if(pick==5&&unlocked>=3)selected=MINE;}else if(pick==6)skill=1;else if(pick==7)paused=true;invalidate();return true;}
