...
FrameLayout.LayoutParams bp=new FrameLayout.LayoutParams(260,100,Gravity.RIGHT|Gravity.BOTTOM);
...
if(continueBtn!=null)continueBtn.setVisibility(View.VISIBLE);
}
);
...
static final int SUNFLOWER=1, PEASHOOTER=2, GIGANUT=3, CHOMPER=4, REPEATER=5, MINE=6, BINU=7;
...
int binuRow=-1,binuCol=-1,binuFrame=0;
long binuClock=0;
boolean binuPlaced=false,binuActive=false;
...
for(int r=0;
r<ROWS;
r++) if(mowers[r]!=null) mowers[r].x=left-cw*.4f;
}
...
c.drawRect(getWidth()*.18f,getHeight()*.215f,getWidth()*.82f,getHeight()*.23f,p);
...
int[] t={
SUNFLOWER,PEASHOOTER,GIGANUT,CHOMPER,REPEATER,MINE,BINU
}
;
for(int i=0;
i<t.length;
i++){
float x=getWidth()*(.01f+i*.063f),y=getHeight()*.075f,w=getWidth()*.055f,h=getHeight()*.09f;
...
c.drawRoundRect(getWidth()*.64f,getHeight()*.075f,getWidth()*.75f,getHeight()*.15f,8,8,p);
text(c,"MUA PF",getWidth()*.695f,getHeight()*.122f,12,Color.WHITE,Paint.Align.CENTER);
}
...
col<COLS;
col++){
if(!activeRow(r)) p.setColor(Color.rgb(72,110,62));
else p.setColor((r+col)%2==0?Color.rgb(103,166,78):Color.rgb(91,153,67));
...
if(binuPlaced&&binuRow>=0&&binuCol>=0&&!binuActive){
float x=left+binuCol*cw+cw/2,y=top+binuRow*ch+ch/2;
drawPlant(c,BINU,x,y,Math.min(cw,ch)*.74f,null);
}
}
...
if(b==null&&a!=null&&type==PEASHOOTER&&a.animFrame>0&&a.animFrame<=10&&peaAnim[a.animFrame-1]!=null)b=peaAnim[a.animFrame-1];
if(b==null){
...
else if(type==BINU)b=binuImg;
}
if(b!=null){
c.drawBitmap(b,null,dr(x-size/2,y-size/2,x+size/2,y+size/2),p);
return;
}
...
if(z.hp<=0)continue;
float w=z.boss?cw*1.35f:(z.giga?cw*.95f:cw*.68f),h=z.boss?ch*1.35f:(z.giga?ch*1.08f:ch*.82f);
if(zombieImg!=null)c.drawBitmap(zombieImg,null,dr(z.x-w/2,z.y-h*.55f,z.x+w/2,z.y+h*.45f),p);
else{
p.setColor(Color.DKGRAY);
c.drawOval(dr(z.x-w/2,z.y-h/2,z.x+w/2,z.y+h/2),p);
}
...
for(Pea q:peas){
float s=q.big?18:10;
if(bulletImg!=null)c.drawBitmap(bulletImg,null,dr(q.x-s,q.y-s,q.x+s,q.y+s),p);
else{
p.setColor(Color.GREEN);
...
for(SunDrop s:suns){
if(sunImg!=null)c.drawBitmap(sunImg,null,dr(s.x-18,s.y-18,s.x+18,s.y+18),p);
else{
...
p.setColor(m.used?Color.DKGRAY:Color.rgb(175,110,45));
c.drawRoundRect(m.x-cw*.30f,y-ch*.18f,m.x+cw*.30f,y,8,8,p);
p.setColor(Color.DKGRAY);
c.drawCircle(m.x-cw*.16f,y+3,cw*.07f,p);
c.drawCircle(m.x+cw*.16f,y+3,cw*.07f,p);
p.setStyle(Paint.Style.STROKE);
p.setStrokeWidth(4);
c.drawLine(m.x+cw*.18f,y-ch*.18f,m.x+cw*.34f,y-ch*.38f,p);
c.drawLine(m.x+cw*.34f,y-ch*.38f,m.x+cw*.22f,y-ch*.50f,p);
...
if(max>0){
p.setColor(Color.GREEN);
c.drawRect(x,y,x+w*Math.max(0,Math.min(1,h/max)),y+6,p);
}
}
void update(){
...
long cd=f?1800:7000;
if(now-a.last>=cd){
suns.add(new SunDrop(left+col*cw+cw/2,top+r*ch+ch*.2f));
a.last=now;
}
}
...
void explodeMine(int row,int col,Plant mine){
float cx=left+col*cw+cw/2f;
for(Zombie z:zombies){
if(z.row>=Math.max(0,row-1)&&z.row<=Math.min(ROWS-1,row+1)&&Math.abs(z.x-cx)<=cw*1.55f)z.hp-=1800;
}
mine.hp=0;
}
...
col++){
Plant a=plants[r][col];
if(a!=null&&a.type==PEASHOOTER&&a.animFrame>0&&now-a.animClock>=55){
a.animClock=now;
a.animFrame++;
if(a.animFrame>10)a.animFrame=0;
}
}
if(!binuPlaced)return;
if(!binuActive){
Zombie target=null;
float bx=left+binuCol*cw+cw/2f;
for(Zombie z:zombies)if(z.hp>0&&z.row>=Math.max(0,binuRow-1)&&z.row<=Math.min(ROWS-1,binuRow+1)&&z.row==binuRow&&z.x>=bx-cw*.15f&&z.x<=bx+cw*1.15f){
target=z;
break;
}
if(target!=null){
...
float bx=left+binuCol*cw+cw/2f;
for(Zombie z:zombies)if(z.hp>0&&z.row>=Math.max(0,binuRow-1)&&z.row<=Math.min(ROWS-1,binuRow+1)&&Math.abs(z.x-bx)<=cw*1.25f){
if(z.boss)z.hp=Math.max(1,(int)(z.hp*0.5f));
else z.hp=0;
}
}
void playBinuSound1(){
try{
if(binuSound1!=null){
if(binuSound1.isPlaying())binuSound1.pause();
binuSound1.seekTo(0);
binuSound1.start();
}
}
...
Zombie hit=null;
for(Zombie z:zombies)if(z.hp>0&&z.row==q.row&&Math.abs(z.x-q.x)<cw*.3f){
hit=z;
break;
}
if(hit!=null){
...
peas.add(new Pea(z.x-cw*.55f,z.y,z.row,150,true,true));
z.lastShot=now;
}
...
r++)for(int col=0;
col<COLS;
col++) if(plants[r][col]!=null&&plants[r][col].hp<=0)plants[r][col]=null;
}
void spawnZombie(){
int n=activeRows(), start=(ROWS-n)/2;
int r=start+rnd.nextInt(n);
boolean boss=level==9&&spawned==total-1;
boolean giga=!boss&&level>=2&&rnd.nextInt(4)==0;
zombies.add(new Zombie(r,getWidth()+cw,top+r*ch+ch/2,boss,giga));
}
long spawnDelay(){
return level<=2?4200:level<=4?3600:level<=8?3100:2600;
}
boolean rowHasZombie(int r){
for(Zombie z:zombies)if(z.row==r&&z.x>left)return true;
return false;
}
...
clear();
last=spawnClock=lastSun=System.currentTimeMillis();
}
void clear(){
for(int r=0;
r<ROWS;
...
void winLevel(){
screen=WIN;
if(level<9)unlocked=Math.max(unlocked,level+1);
save();
if(level==9)post(MainActivity.this::showWinVideo);
}
void useFood(Plant a){
if(a==null||food<=0||a.foodUsed)return;
food--;
a.foodUsed=true;
playFoodSound();
save();
...
void save(){
getSharedPreferences("garden_defense", MODE_PRIVATE).edit() .putInt("level",level) .putInt("unlocked",unlocked) .putInt("coins",coins) .putInt("food",food) .apply();
}
void releaseSounds(){
if(foodSound!=null){
foodSound.release();
...
if(i<=unlocked&&x>=x1&&x<=x1+getWidth()*.17f&&y>=y1&&y<=y1+getHeight()*.13f){
startLevel(i);
return true;
}
}
...
if(screen==PLAY){
if(y>=getHeight()*.075f&&y<=getHeight()*.15f&&x>=getWidth()*.64f&&x<=getWidth()*.75f){
buyFood();
invalidate();
return true;
}
...
for(Iterator<SunDrop> it=suns.iterator();
it.hasNext();
){
SunDrop s=it.next();
...
static class Plant {
public static final int SUNFLOWER=1,PEASHOOTER=2,GIGANUT=3,CHOMPER=4,REPEATER=5,MINE=6;
public int type,row,col,hp,maxHp;
public long last,secondShot,foodUntil,armAt,animClock;
public int animFrame=0;
public boolean foodUsed,armed;
public Plant(int type,int row,int col){
this.type=type;
this.row=row;
this.col=col;
maxHp=type==GIGANUT?4000:1000;
hp=maxHp;
...
Zombie(int row,float x,float y,boolean boss,boolean giga){
this.row=row;
this.x=x;
this.y=y;
this.boss=boss;
this.giga=giga;
maxHp=boss?2500:(giga?900:300);
hp=maxHp;
speed=boss?12:(giga?16:22);
damage=boss?150:100;
}
}
class Pea {
...
int row,damage;
boolean big,enemy;
Pea(float x,float y,int row,int damage,boolean big){
this(x,y,row,damage,big,false);
}
Pea(float x,float y,int row,int damage,boolean big,boolean enemy){
this.x=x;
this.y=y;
this.row=row;
this.damage=damage;
